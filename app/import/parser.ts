...
function inferCategory(text: string): Category | null {
  ...
  if (
    (t.includes('cfexpress') && t.includes('type a')) ||
    t.includes('cfexpress type a')
  ) {
    return 'cfexpress_a';
  }

  if (
    (t.includes('cfexpress') && t.includes('type b')) ||
    t.includes('cfexpress type b')
  ) {
    return 'cfexpress_b';
  }
...
function extractVpgClass(text: string, category: Category): VpgClass {
  if (category === 'sd') return 'unknown';

  const t = normalizeText(text).toUpperCase();

  if (/\bVPG[\s-]?1600\b/.test(t)) return 'VPG1600';
  if (/\bVPG[\s-]?800\b/.test(t)) return 'VPG800';
  if (/\bVPG[\s-]?400\b/.test(t)) return 'VPG400';
  if (/\bVPG[\s-]?200\b/.test(t)) return 'VPG200';

  if (
    t.includes('VIDEO PERFORMANCE GUARANTEE 1600') ||
    t.includes('VIDEO PERFORMANCE GUARANTEE-1600')
  ) {
    return 'VPG1600';
  }
  ...
  if (
    t.includes('VIDEO PERFORMANCE GUARANTEE 800') ||
    t.includes('VIDEO PERFORMANCE GUARANTEE-800')
  ) {
    return 'VPG800';
  }
  ...
  if (
    t.includes('VIDEO PERFORMANCE GUARANTEE 400') ||
    t.includes('VIDEO PERFORMANCE GUARANTEE-400')
  ) {
    return 'VPG400';
  }
  ...
  if (
    t.includes('VIDEO PERFORMANCE GUARANTEE 200') ||
    t.includes('VIDEO PERFORMANCE GUARANTEE-200')
  ) {
    return 'VPG200';
  }
...
const cardHtmlNormalized = normalizeText(cardHtml);

const combinedForCategory = normalizeText([name, cardText, cardHtmlNormalized].join(' '));
const category = inferCategory(combinedForCategory);
if (!category) return null;
  ...
  const bhMatch = cardText.match(/BH\s*#\s*([A-Z0-9-]+)/i);
  const bhNumber =
    bhMatch?.[1] ?? firstAttr(card, ['data-item-id', 'data-sku']) ?? '';

  const modelMatch = cardText.match(/MFR\s*#\s*([A-Z0-9._/-]+)/i);
...
const vpgClass = extractVpgClass(vpgSourceText, category);

if (category === 'cfexpress_a' && vpgClass === 'non_vpg') {
  console.log('[VPG missing on Type A]', {
    name,
    model,
    bhNumber,
    previewText: cardText.slice(0, 300),
    previewHtml: cardHtml.slice(0, 500),
  });
}
...
return {
  ...
  model,
  url,
  videoSpeedClass: extractVideoSpeedClass(vpgSourceText),
  vpgClass: extractVpgClass(vpgSourceText, category),
  vpgFacet: inferVpgFacetFromUrl(url),
  isKit,
  kitSize,
  regularPrice,
  salePrice,
  isOnSale,
  stockStatus: detectStock(cardText),
};
}
